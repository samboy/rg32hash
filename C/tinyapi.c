...
){rg s,q,c;rgp(40)w[c]=u[c%19]=0;for(;;rgf(u,w)){rgp(3){for(q=0;q<4;)
{w[c*13]^=s=(*v?255&*v:1)<<8*q++;u[16+c]^=s;if(!*v++){rgp(17)rgf(u,w)
;return;}}}}}rg rn(rg*m,rg*b,rg*a){if(*a&2)rgf(m,b);return m[*a^=3];}
...
int main(int argc,char **argv){
        ...
        for(c = 0; c < 8; c++) {
                j = rn(mill, belt, &phase); /* Get number from PRNG */
