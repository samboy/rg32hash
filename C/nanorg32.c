...
c]=k>>j|k<<(32-j);}for(c--;c--;f[c+1]=f[c])e[c]=n[c]^n[(c+1)%g]^n[(c+4)%
g];*e^=1;b(3)e[c+h]^=f[c*h]=f[c*h+h];}int main(int p,char**v){char *q=v[
--p];b(40)f[c]=e[c]=0;for(;;m()){b(3){for(p=0;p<4;){f[c*h]^=k=(*q?255&*q
:1)<<8*p++;e[16+c]^=k;if(!*q++){b(17)m();p=2;b(8){if(p&2)m();k=e[p^=3];i
