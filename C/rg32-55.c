...
void dwr_input_map(DWR_WORD *a, DWR_WORD *b, uint64_t length) {
	DWR_WORD p[3];
	int q = 0;
	int c = 0;
	int r = 0;
...
/* Given a length, make a rg32 state of a hash with 0x55 (“U”) multipled
 * by length */
dwr_rg *dwr_init_rg(uint64_t length) {
	dwr_rg *out;
	int c = 0;
	...
	out->index = 0;
	dwr_input_map(out->a,out->b,length);
...
int main(int argc, char **argv) {
	...
	uint64_t length;
	if(argc != 2) {
		printf("Usage: rg32 {length of hash input}\n");
		exit(1);
	}
	length = atoll(argv[1]);
	hash = dwr_init_rg(length);
	if(hash == 0) {
		exit(1);
	}
	for(a=0;a<8;a++) {
