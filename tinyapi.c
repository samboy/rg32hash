...
+o]^=q[c];*a^=1;}void rgl(rgp*u,rgp*w,char*v){rgp s,q,c,x;rgf(39)w[
c]=u[c%19]=0;for(;;rg(u,w)){rgf(3){for(s=q=0;q<4;){x=*v++;s|=(x?255
&x:1)<<8*q++;if(!x){rgn;rgf(17)rg(u,w);return;}}rgn;}}}rgp rgi(rgp*
m,rgp*b,rgp*a){if(*a&2)rg(m,b);return m[*a^=3];}
