/* Donated the the public domain 2012, 2015, and 2018 Sam Trenholme.  
 * Written in 2009, 2012, 2015, and 2018.
 *
...
void dwr_update_iota(DWR_WORD *iota) {
	uint32_t field = 1;
	if(*iota & 1) {
		*iota >>= 1;
		*iota ^= field;
...
void dwr_mill(DWR_WORD *a, DWR_WORD *iota) {
	...
	a[0] ^= *iota;
	dwr_update_iota(iota);
}	

/* The "belt" part of Radio Gatun */
void dwr_belt(DWR_WORD *a, DWR_WORD *b, DWR_WORD *iota) {
...
void dwr_input_map(DWR_WORD *a, DWR_WORD *b, DWR_WORD *iota, uint8_t *v) {
	DWR_WORD p[3];
	int q = 0;
	int c = 0;
	int r = 0;
...
dwr_rg *dwr_init_rg(char *v) {
	...
	out->index = 0;
	dwr_input_map(out->a,out->b,&(out->iota),v);
	for(c = 0; c < DWR_BLANKROUNDS; c++) {
		dwr_belt(out->a,out->b,&(out->iota));
	}
	return out;
}
...
DWR_WORD dwr_rng(dwr_rg *state) {
	DWR_WORD out;
	if((state->index & 1) == 0) {
		dwr_belt(state->a,state->b,&(state->iota));
		out = state->a[1];	
	} else {
		out = state->a[2];
	}
